...
def create_header():
  ...
  with ui.header().classes('items-center justify-between bg-blue-600'):
    ...
    with ui.row().classes('gap-2'):
      ...
      ui.button('Clothes', icon='checkroom', on_click=lambda: ui.navigate.to('/clothes')).props('flat color=white')
      ui.button('Configurator', icon='settings', on_click=lambda: ui.navigate.to('/configurator')).props('flat color=white')
      ui.button('Machines', icon='local_laundry_service', on_click=lambda: ui.navigate.to('/machines')).props('flat color=white')
...
def home_page():
  ...
  with ui.column().classes('w-full p-8 items-center'):
    ...
    ui.label('Your smart clothes management system').classes('text-xl text-gray-600 mb-8')
    
    # Stats cards
    with ui.row().classes('gap-4 mb-8'):
      with ui.card().classes('w-48 h-32 flex flex-col justify-center items-center bg-blue-50'):
        ui.label('Total Clothes').classes('text-sm text-gray-600')
        ui.label('0').classes('text-4xl font-bold text-blue-600')
      
      with ui.card().classes('w-48 h-32 flex flex-col justify-center items-center bg-green-50'):
        ui.label('Washing Modes').classes('text-sm text-gray-600')
        ui.label('0').classes('text-4xl font-bold text-green-600')
      
      with ui.card().classes('w-48 h-32 flex flex-col justify-center items-center bg-purple-50'):
    ...
    with ui.row().classes('gap-4'):
      ui.button('Add New Clothes', icon='add', on_click=lambda: ui.navigate.to('/clothes')).classes('bg-blue-600')
      ui.button('Start Washing Session', icon='play_arrow', on_click=lambda: ui.navigate.to('/configurator')).classes('bg-green-600')
...
def clothes_page():
  ...
  with ui.column().classes("w-full p-8"):
    with ui.row().classes("w-full items-center justify-between mb-6"):
      ...
      ui.button("Add New Item", icon="add", on_click=lambda: add_clothes_dialog.open()).classes("bg-blue-600")
    
    # Search bar
    ui.input("Search clothes...", placeholder="Search by name, fabric, or color").classes("w-full mb-4").props("outlined")
    ...
    with ui.column().classes("w-full items-center justify-center p-16"):
      ...
      ui.label("Add your first clothing item to get started").classes("text-sm text-gray-400")
  ...
  with ui.dialog() as add_clothes_dialog, ui.card().classes("w-96"):
    ...
    with ui.row().classes("w-full justify-end mt-4 gap-2"):
      ...
      ui.button("Add Item", on_click=lambda: [ui.notify("Item added!"), add_clothes_dialog.close()]).classes("bg-blue-600")
  ...
  with ui.column().classes("w-full p-8"):
    ...
    with ui.row().classes("w-full gap-8"):
      ...
      with ui.card().classes("flex-1"):
        ...
        ui.label("Check the items you want to wash:").classes("text-sm text-gray-600 mb-4")
      ...
      with ui.card().classes("flex-1"):
        ...
        with ui.column().classes("w-full items-center p-8"):
          ui.icon("check_circle_outline", size="3rem").classes("text-gray-300 mb-2")
          ui.label("No items selected yet").classes("text-gray-400 italic")
    
    ui.separator().classes("my-6")
    
    # Analysis button
    ui.button("Analyze & Generate Washing Plan", 
             icon="psychology",
             on_click=lambda: ui.notify("Please select items first", type="warning")).classes("bg-green-600 text-lg")
    ...
    with ui.card().classes("w-full bg-gray-50"):
      with ui.column().classes("w-full items-center p-12"):
        ...
        ui.label("Washing plan will appear here").classes("text-xl text-gray-400 mb-2")
        ui.label("Select clothes and click \"Analyze\" to generate a plan").classes("text-sm text-gray-400")
...
def machines_page():
  ...
  with ui.column().classes("w-full p-8"):
    with ui.row().classes("w-full items-center justify-between mb-6"):
      ...
      ui.button("Add Mode", icon="add", on_click=lambda: add_mode_dialog.open()).classes("bg-blue-600")
    
    # Empty state
    with ui.column().classes("w-full items-center justify-center p-16"):
      ui.icon("local_laundry_service", size="4rem").classes("text-gray-300 mb-4")
      ui.label("No washing modes configured").classes("text-xl text-gray-400 mb-2")
      ui.label("Add your washing machine modes to get started").classes("text-sm text-gray-400")
  ...
  with ui.dialog() as add_mode_dialog, ui.card().classes("w-96"):
    ...
    with ui.row().classes("w-full justify-end mt-4 gap-2"):
      ...
      ui.button("Add Mode", on_click=lambda: [ui.notify("Mode added!"), add_mode_dialog.close()]).classes("bg-blue-600")
